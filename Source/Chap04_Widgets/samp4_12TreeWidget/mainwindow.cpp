...
void MainWindow::iniTree() {
  ...
  QTreeWidgetItem *item =
      new QTreeWidgetItem(MainWindow::itTopItem);     //节点类型为itTopItem
...
void MainWindow::addFolderItem(QTreeWidgetItem *parItem, QString dirName) {
  ...
  QString NodeText =
      getFinalFolderName(dirName); //从一个完整目录名称里，获取最后的文件夹名称

  QTreeWidgetItem *item =
      new QTreeWidgetItem(MainWindow::itGroupItem); //设定type为itGroupItem
  ...
  item->setData(
      colItem, Qt::UserRole,
      QVariant(dirName)); //设置角色为Qt::UserRole的Data,存储完整目录名称
...
void MainWindow::addImageItem(QTreeWidgetItem *parItem, QString aFilename) {
  ...
  //    QTreeWidgetItem *item;
  QTreeWidgetItem *item =
      new QTreeWidgetItem(MainWindow::itImageItem); //节点类型为itImageItem
  ...
  item->setData(colItem, Qt::UserRole,
                QVariant(aFilename)); //设置用户数据,存储完整文件名称
  parItem->addChild(item);            //在父节点下面添加子节点
}
...
void MainWindow::displayImage(QTreeWidgetItem *item) {
  QString filename =
      item->data(colItem, Qt::UserRole).toString(); //获取节点data里存的文件名
...
void MainWindow::on_actAddFiles_triggered() {
  QStringList files =
      QFileDialog::getOpenFileNames(this, "选择文件", "", "Images(*.jpg)");
...
void MainWindow::on_dockWidget_visibilityChanged(
    bool visible) { //停靠区可见性变化
  ui->actDockVisible->setChecked(visible);
}

void MainWindow::on_dockWidget_topLevelChanged(
    bool topLevel) { //停靠区浮动性变化
  ui->actDockFloat->setChecked(topLevel);
}

void MainWindow::on_actDockFloat_triggered(bool checked) { //停靠区浮动性
  ui->dockWidget->setFloating(checked);
}

void MainWindow::on_actDockVisible_triggered(bool checked) { //停靠区的可见性
